/**
 * Seed Question Bank v3 - Gera questoes para o 3o ano via Gemini
 *
 * Uso: source <(cat .env.local | sed "s/^/export /") && npx tsx scripts/seed_questions_v3.ts
 */
import fs from "fs"
...
async function main() {
  const jsonPath = path.join(__dirname, "curriculum_enrichment_3ano.json")
  if (!fs.existsSync(jsonPath)) {
    console.error("Arquivo curriculum_enrichment_3ano.json nao encontrado!")
    console.error("Rode: python3 scripts/pipeline_enrich.py scripts/1761104492059_Programacao_Trimestral_Editavel3A.xlsx --output curriculum_enrichment_3ano.json")
    process.exit(1)
  }
  
  const entries: EnrichmentEntry[] = JSON.parse(fs.readFileSync(jsonPath, "utf-8"))
  console.log(`Unidades carregadas: ${entries.length}`)

  // Check existing questions for 3o ano
  const existing = await sql`SELECT COUNT(*) as c FROM question_bank WHERE grade_year = 3 AND source = 'seed_ia'`
  const existingCount = Number(existing[0].c)
  if (existingCount > 0) {
    console.log(`Ja existem ${existingCount} questoes seed_ia para 3o ano.`)
    console.log("Para limpar: DELETE FROM question_bank WHERE grade_year=3 AND source='seed_ia';")
  }

  const existingRows = await sql`SELECT DISTINCT subject, bimester FROM question_bank WHERE grade_year = 3 AND source = 'seed_ia'`
  ...
  for (const entry of entries) {
    ...
    console.log(`\nGerando ${subject} - ${entry.chapter.substring(0, 60)}...`)
    const questions = await generateQuestions(entry, 3)
    if (questions.length === 0) { console.log("  Nenhuma questao gerada"); continue }
