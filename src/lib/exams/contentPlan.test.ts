...
const units: CurriculumSelection['units'] = [
  { rowIndex: 0, bimestre: '2', tituloCapitulo: 'Semelhança de triângulos', conteudo: 'Geometria plana', habilidades: { status: 'nao_mapeado', skills: [] }, objetivos: [], objetivosColumnMissing: false },
...
describe('contentPlan', () => {
  it('mantém apenas os capítulos selecionados e exige a soma definida', () => {
    const result = validateCurriculumPlan(units, [
      { unitRowIndex: 0, questionCount: 3, priority: 'alta', visualAid: 'auto' },
      { unitRowIndex: 5, questionCount: 0, priority: 'baixa', visualAid: 'sem_imagem' },
    ], 3)
    expect(result.selectedUnits.map((unit) => unit.rowIndex)).toEqual([0])
    expect(() => validateCurriculumPlan(units, [{ unitRowIndex: 0, questionCount: 2, priority: 'alta', visualAid: 'auto' }], 3)).toThrow('soma 2')
  })
