...
function question(overrides: Partial<ExamQuestion> = {}): ExamQuestion {
  return {
    ...
    alternatives: [
      { letter: 'A', text: 'Uma unidade isolada.' },
      { letter: 'B', text: 'Unidades próximas e interligadas.' },
      { letter: 'C', text: 'Expansão urbana.' },
      { letter: 'D', text: 'Caça controlada.' },
      { letter: 'E', text: 'Plantações comerciais.' },
    ],
    correctLetter: 'B', expectedAnswer: null, gradingCriteria: null,
...
describe('correctSingleQuestion', () => {
  ...
  it('remove texto de apoio que define o conceito cobrado', () => {
    const result = correctSingleQuestion(question(), curriculum)
    expect(result.question.supportText).toBeNull()
    expect(result.warnings).toContain('Questão 5: texto de apoio definia o conceito cobrado e foi removido para não entregar a resposta.')
  })

  it('preserva evidência que não define o conceito da pergunta', () => {
